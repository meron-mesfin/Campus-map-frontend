...
export function ManageAdmins() {
  ...
  const validate = (field, value) => {
    ...
    if (field === 'password') {
      if (!editingAdmin && (!value || !value.trim())) {
        newErrors.password = 'Password is required';
      } else if (value && value.trim()) {
        const strongPasswordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#\-_])[A-Za-z\d@$!%*?&#\-_]{8,}$/;
        if (!strongPasswordRegex.test(value)) {
          newErrors.password = 'Must be at least 8 characters with 1 uppercase, 1 lowercase, 1 number, and 1 special character (@$!%*?&#\\-_)';
        } else {
  ...
  const validateForm = () => {
    ...
    if (!editingAdmin && (!formData.password || !formData.password.trim())) {
      ...
      if (!strongPasswordRegex.test(formData.password)) {
        tempErrors.password = 'Must be at least 8 characters with 1 uppercase, 1 lowercase, 1 number, and 1 special character (@$!%*?&#\\-_)';
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={editingAdmin ? "Edit Admin" : "Create New Admin"}>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
              {editingAdmin ? "New Password (Optional)" : "Temporary Password"} {!editingAdmin && <span className="text-red-500">*</span>}
            </label>
            <input
              ...
              placeholder={editingAdmin ? "Leave blank to keep current" : "Min 8 chars, 1 upper, 1 lower, 1 digit, 1 special"}
              className={`w-full px-4 py-2 rounded-lg border bg-white dark:bg-slate-900 text-slate-900 dark:text-white outline-none focus:ring-2 ${
                errors.password
                  ? 'border-red-500 focus:ring-red-500 shadow-sm shadow-red-100 dark:shadow-none'
                  : 'border-slate-300 dark:border-slate-600 focus:ring-primary-500'
